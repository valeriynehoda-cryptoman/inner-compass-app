...
export default function App() {
  ...
  const handleBuyClick = async () => {
    ...
    try {
      ...
      if (!userId) {
        alert('Не удалось определить ID пользователя Telegram. Откройте приложение из бота.');
      ...
      if (data.success) {
        // Закрываем мини-приложение, чтобы счет появился прямо в чате
              ...
              <div className="absolute inset-0 bg-slate-950/85 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center z-10 space-y-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    Индивидуальный план выхода из кризиса, триггеры и персональные практики (250 Stars)
                  </p>
                </div>
                <button
                ...
                >
                  {loadingPayment ? 'Отправка счета в чат...' : 'Разблокировать за 250 ⭐'}
                </button>
              </div>
